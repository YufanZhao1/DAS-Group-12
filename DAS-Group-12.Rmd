---
title: "Find the factor of good quality coffee!"
author: "Group 12"
...
In gengeral, we can find that the distributions of country of origin, year harvested and category of two defect are not significantly different. But we need to investigate more to decide the final model.
...
Then, we exclude three insignificant variables(country of origin, year harvested and category of two defect) one by one to build 3 models and compare them.
...
To choose the best fit for our model, we choose the model with low AIC, BIC and large deviance. From the table above, we can see that model 1 has the highest BIC, so model 3 is the best fit compared to the erst.
